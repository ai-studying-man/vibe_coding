...
- `character_shadow`: 글자그림자. `params.type`, `params.color`, `params.offset_x`, `params.offset_y`로 문자 그림자 스타일을 지정한다.
- `text_background`: 글자 배경색
- `strike_or_underline`: 취소선/밑줄
- `paragraph_alignment`: 문단 정렬
- `paragraph_spacing`: 문단 간격
- `paragraph_indent`: 문단 들여쓰기/내어쓰기. `params.indent`는 복원 소스와 같은 포인트 기준 값이며 HWPUNIT으로 변환된다.
- `paragraph_block_spacing`: 문단 위/아래 간격. `params.prev`, `params.next`는 복원 소스와 같은 포인트 기준 값이며 HWPUNIT으로 변환된다.
